...
function Videos() {
  ...
  const videosArr = [
    ...
    'https://player.vimeo.com/progressive_redirect/playback/690770660/rendition/720p?loc=external&oauth2_token_id=1027659655&signature=7a048039957fd878fc72b809b9a0e5f2102eded879a83e00784ecd3ba5123614',
    
  ];
  const [videoSrc, setVideoSrc] = useState(videosArr[0]);
  useEffect(() => {
    document.title = 'Renis - YouTube';
  }, []);

  return (
    <Stack p={20} h={"150vh"} direction={["column","row"]}>
        <VStack w={"full"}>
            <video 
              controls
        ...
        <VStack w={["full" , "xl"]} alignItems={"stretch"} p={"8"} spacing={"8"} overflowY={"auto"}>
          <Heading>All Video</Heading>
          {
            ...
            <VStack  >
              <video 
                controls
                controlsList='nodownload' 
                src={item} style={{width:"240px"}}></video>
            </VStack>
            <Link onClick={() => setVideoSrc(item)}>
            <VStack >
                  <Heading fontSize={"15"} fontWeight={"semibold"}>Sample Video {index + 1}</Heading>
                  <Text fontSize={"15"} p={"1"}>This is a sample video for testing demo.</Text>
            </VStack>
            </Link>
          </Stack>
            ))
          ...
          {/* <Stack display={"flex"} flexDirection={"row"} >
            ...
            <Link>
            <VStack >
                  <Heading fontSize={"20"} fontWeight={"semibold"}>Sample Video 2</Heading>
                  <Text fontSize={"15"} p={"1"}>This is a sample video for testing demo.</Text>
            </VStack>
            </Link>
          </Stack>
          <Stack display={"flex"} flexDirection={"row"}>
            ...
            <Link>
            <VStack >
                  <Heading fontSize={"20"} fontWeight={"semibold"}>Sample Video 3</Heading>
                  <Text fontSize={"15"} p={"1"}>This is a sample video for testing demo.</Text>
            </VStack>
            </Link>
          </Stack>
          <Stack display={"flex"} flexDirection={"row"}>
            ...
            <Link>
            <VStack >
                  <Heading fontSize={"20"} fontWeight={"semibold"}>Sample Video 4</Heading>
                  <Text fontSize={"15"} p={"1"}>This is a sample video for testing demo.</Text>
            </VStack>
            </Link>
          </Stack> */}
